...
function backup_folder {
    ...
    for file in *; do
      if [ "$file" != "$backup_dir" ] && [ "$file" != "production_backup_and_build.sh" ] && [ "$file" != "Web.config" ]; then
        mv "$file" "$backup_dir"
      fi
    done
...
function pull_and_build {
    ...
    for file in *; do
      if [ "$file" != "bak" ] && [ "$file" != "production_backup_and_build.sh" ] && [ "$file" != "Web.config" ] && [ "$file" != "build" ]; then
        if [ -d "$file" ]; then
...
# read -p "Which branch would you like to pull and build? (production/staging/production): " branch_name
